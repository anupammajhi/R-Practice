...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood$Movie[ which( bollywood$Tcollection == sort(bollywood$Tcollection, decreasing = TRUE)[2])] #Works for any rank, needs number to be replaced by rank
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- sapply(bollywood[,4:7], function(x) { bollywood$Movie[which.max(x)]})
